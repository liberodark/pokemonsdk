...
class ::PFM::Wild_RoamingInfo
  ...
  def update
    RoamingProcs[@proc_id].call(self) if RoamingProcs[@proc_id]
  end
  # Test if the Roaming Pokemon is appearing (to start the battle)
  def appearing?
    return false if @pokemon.hp == 0
    if @map_id == $game_map.map_id and 
      @zone_type == $env.get_zone_type(true) and 
  ...
  def could_appear?
    return (@map_id == $game_map.map_id and 
      @zone_type == $env.get_zone_type(true) and 
      @tag == $game_player.terrain_tag)
  end
end
