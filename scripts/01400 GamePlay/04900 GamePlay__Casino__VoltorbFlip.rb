    ...
    module Casino
        class VoltorbFlip < Base
            ...
            def update_input_dir
                case Input.dir4
                when 2; @cursor.move_on_board(0, 1)
                when 4; @cursor.move_on_board(-1, 0)
                when 6; @cursor.move_on_board(1, 0)
                when 8; @cursor.move_on_board(0, -1)
                end
            end
            ...
            def update_tiles_animation
                ...
                unless @no_animation
                    ...
                    if result.is_a?(UI::VoltorbFlip::BoardTile)
                        @animation_sprite.animate(result)
                    end
                end
                return (result == true)
            end
            ...
            def on_memo_activate(y = @cursor.board_y)
                ...
                if y == 4
                    @memo_disabler.set_rect_div(1, 0, 2, 1)
                end
                update_cursor_mode
            end
            ...
            def on_board_activate(index = nil)
                ...
                if @cursor.mode == :normal 
                    ...
                    if @board_tiles[index].content.is_a?(Integer)
                        ...
                        if old_coin_gain != n_coin_gain
                            @coin_gain_increase = get_increment(old_coin_gain, n_coin_gain)
                        end
                    else
                        on_fail
                    end
            ...
            def get_revealed_tile_count
                return @board_tiles.select { |t| t.revealed? }.length
            end
