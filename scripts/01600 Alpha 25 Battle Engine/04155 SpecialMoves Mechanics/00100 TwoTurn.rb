module Battle::Move::Mechanics
  ...
  module TwoTurn
    ...
    def move_usable_by_user(user, targets)
      return move_usable_by_user_turn_1(super, user, targets) unless user.effects.has?(&:force_next_move?)

      return move_usable_by_user_turn_2(super, user, targets)
    end
    ...
    def move_usable_by_user_turn_1(super_result, user, targets)
      return true if check_shortcut_turn_1(user, targets)

      proceed_effects_turn_1(user, targets)
      proceed_message_turn_1(user, targets)
      proceed_animation_turn_1(user, targets)
      return false
    end
    alias two_turn_move_usable_by_user_turn_1 move_usable_by_user_turn_1
    ...
    # @note Thing that prevents the move from being used should be defined by :move_prevention_user Hook
    # @return [Boolean] if the procedure can continue
    def move_usable_by_user_turn_2(super_result, user, targets)
      remove_effects_turn_2(user, targets)
      return false unless super_result

      return true
    end
    alias two_turn_move_usable_by_user_turn_2 move_usable_by_user_turn_2
    ...
    alias two_turn_check_shortcut_turn_1 check_shortcut_turn_1
    ...
    def proceed_effects_turn_1(user, targets)
      ...
      stat_changes_turn_1(user, targets)&.each do |(stat, value)|
        @logic.stat_change_handler.stat_change_with_process(stat, value, user)
      end
    end
    alias two_turn_proceed_effects_turn_1 proceed_effects_turn_1
    ...
    alias two_turn_proceed_message_turn_1 proceed_message_turn_1

    # Display the message and the animation of the turn
    # @param user [PFM::PokemonBattler]
    # @param targets [Array<PFM::PokemonBattler>] expected targets
    def proceed_animation_turn_1(user, targets)
      nil
    end
    alias two_turn_proceed_animation_turn_1 proceed_animation_turn_1
    ...
    alias two_turn_stat_changes_turn_1 stat_changes_turn_1

    # Remove effects on turn 2
    # @param user [PFM::PokemonBattler]
    # @param targets [Array<PFM::PokemonBattler>] expected targets
    def remove_effects_turn_2(user, targets)
      user.effects.get(&:out_of_reach?)&.kill
      user.effects.deleted_dead_effects
    end
    alias two_turn_remove_effect_turn_2 remove_effects_turn_2
